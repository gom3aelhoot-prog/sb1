...
export default function RadiologyPage() {
  ...
  const [bookingCenter, setBookingCenter] = useState<RadiologyCenter | null>(null);
  const [bookForm, setBookForm] = useState({ patient_name: '', patient_email: '', patient_phone: '', service_type: '', scheduled_at: '' });
  ...
  const handleBook = async () => {
    ...
    await supabase.from('radiology_bookings').insert({
    ...
    setBookSuccess(true);
    setTimeout(() => { setBookingCenter(null); setBookSuccess(false); setBookForm({ patient_name: '', patient_email: '', patient_phone: '', service_type: '', scheduled_at: '' }); }, 2000);
  };

  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-6xl mx-auto px-4">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">{t('radiology.title')}</h1>
        <p className="text-gray-500 mb-6">{t('radiology.subtitle')}</p>
            ...
            {centers.map((c) => (
              <div key={c.id} className="card overflow-hidden hover:shadow-lg transition-all">
                ...
                <div className="p-5">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => navigate('/radiology/'+c.id)} className="btn-secondary w-full text-sm">تفاصيل المؤسسة</button><button onClick={()=>addCart({item_type:'radiology',item_id:c.id,name:c.name,image_url:c.image_url,unit_price:25,currency_code:'USD',quantity:1,metadata:{service:c.services||'radiology'}})} className="btn-primary w-full text-sm"><ShoppingCart className="inline w-4 h-4 me-1"/>إضافة للسلة</button><button onClick={() => setBookingCenter(c)} className="btn-primary w-full text-sm flex items-center justify-center gap-2">
            ...
            {bookSuccess ? (
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-gray-800">{t('radiology.book')} - {bookingCenter.name}</h3>
                ...
                <div className="space-y-3">
                  ...
                  <input placeholder={t('radiology.title')} value={bookForm.service_type} onChange={(e) => setBookForm({ ...bookForm, service_type: e.target.value })} className="input-field" />
