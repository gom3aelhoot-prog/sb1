...
export default function WalletPage(){
 ...
 useEffect(()=>{load()},[]);
 useEffect(()=>{if(q.topup==='success'&&q.session_id){setBusy(true);fetch('/api/wallet-confirm',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({session_id:q.session_id,session_key:sessionKey})}).then(r=>r.json()).then(x=>{if(x.ok)setMessage(lang==='ar'?'تم شحن المحفظة بنجاح.':'Wallet topped up successfully.');else setMessage(x.error||'Payment could not be verified.');return load()}).finally(()=>setBusy(false))}},[q.topup,q.session_id]);
