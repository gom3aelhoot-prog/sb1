...
export default function WalletPage(){
 ...
 useEffect(()=>{load()},[]);
 useEffect(()=>{if(q.topup==='success'&&q.session_id){setBusy(true);fetch('/api/wallet-confirm',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({session_id:q.session_id,access_token:sessionKey})}).then(r=>r.json()).then(x=>{if(x.ok)setMessage(lang==='ar'?'تم شحن المحفظة بنجاح.':'Wallet topped up successfully.');else setMessage(x.error||'Payment could not be verified.');return load()}).finally(()=>setBusy(false))}},[q.topup,q.session_id]);
 const topup=async()=>{const n=Number(amount);if(!n||n<=0)return;setBusy(true);try{const r=await fetch('/api/wallet-checkout',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({amount:n,currency:(wallet?.currency_code||'USD').toLowerCase(),access_token:token})});const x=await r.json();if(!r.ok)throw new Error(x.error);location.href=x.url}catch(e){setMessage(e instanceof Error?e.message:'تعذر بدء الدفع')}finally{setBusy(false)}};
