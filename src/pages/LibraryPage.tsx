...
export default function LibraryPage(){
 ...
 return <div dir={dir} className="min-h-screen bg-gray-50 pt-24 pb-16"><div className="mx-auto max-w-7xl px-4">
  ...
  <div className="my-5 grid grid-cols-2 gap-2 md:grid-cols-4">{[['news',Newspaper,t.news],['services',Wrench,t.services],['books',BookOpen,t.books],['interactions',Users,t.interactions]].map((x:any)=>{const TabIcon=x[1];return <button key={x[0]} onClick={()=>setTab(x[0])} className={`rounded-xl px-4 py-3 font-bold ${tab===x[0]?'bg-teal-600 text-white':'border bg-white text-gray-700'}`}><TabIcon className="mx-auto mb-1 h-5 w-5"/>{x[2]}</button>})}</div>
