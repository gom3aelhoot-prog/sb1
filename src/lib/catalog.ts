...
export function virtualVideosForSpecialty(slug:string,lang:string,count=2):DoctorVideo[]{
 ...
 return Array.from({length:count},(_,i)=>({id:`catalog-vid-${lang}-${slug}-${i+1}`,doctor_id:doctor.id,specialty_id:specialty.id,title:`${L(lang).video}: ${localizedSpecialty(s,lang)} — ${i+1}`,description:L(lang).articleIntro,video_url:i%2===0?'https://collections.nlm.nih.gov/ext/hmdvid/hmdvid-8801174A/hmdvid-8801174A.m4v':'https://d34w7g4gy10iej.cloudfront.net/video/2609/DOD_111975192/DOD_111975192.mp4',thumbnail_url:'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=900&q=80',duration_seconds:180,views:500+i*100,created_at:new Date(2026,1,5+i).toISOString(),doctor,specialty:specialty}));
}
export function virtualAudioForSpecialty(slug:string,lang:string,count=3):DoctorAudio[]{const s=spec(slug);if(!s)return[];const specialty=sp(lang,slug);const doctor=virtualDoctorsForSpecialty(slug,lang,5)[0];return Array.from({length:count},(_,i)=>({id:`catalog-audio-${lang}-${slug}-${i+1}`,doctor_id:doctor.id,specialty_id:specialty.id,title:`${L(lang).audio}: ${localizedSpecialty(s,lang)} — ${i+1}`,description:L(lang).articleIntro,audio_url:i%2===0?'https://upload.wikimedia.org/wikipedia/commons/transcoded/0/0a/Frederic_Lardon_-_01_-_Hand_Pain.ogg/Frederic_Lardon_-_01_-_Hand_Pain.ogg.mp3':'https://upload.wikimedia.org/wikipedia/commons/transcoded/7/7a/HROgg.ogg/HROgg.ogg.mp3',duration_seconds:300,listens:200+i*30,created_at:new Date(2026,1,10+i).toISOString(),doctor,specialty:specialty}));}
export function virtualLibraryForSpecialty(slug:string,lang:string,count=3):SpecialtyLibraryItem[]{const s=spec(slug);if(!s)return[];const specialty=sp(lang,slug);return Array.from({length:Math.max(7,count)},(_,i)=>({id:`catalog-book-${lang}-${slug}-${i+1}`,specialty_id:specialty.id,item_type:'book',title:`${L(lang).book}: ${localizedSpecialty(s,lang)} — ${i+1}`,description:L(lang).articleIntro,url:null,image_url:null,source:'SB1 AI Editorial Library',is_auto_generated:true,created_at:new Date(2026,1,15+i).toISOString(),specialty}));}
...
const FEATURED_COURSE_TOPICS=[
 {key:'aba',ar:'التحليل التطبيقي السلوكي ABA — برنامج مهني شامل',en:'Applied Behavior Analysis (ABA) — Comprehensive Program',de:'Angewandte Verhaltensanalyse (ABA) — Komplettprogramm',ru:'Прикладной анализ поведения (ABA) — комплексная программа'},
 {key:'neuropsych',ar:'علم النفس العصبي والتقييم النفسي العصبي',en:'Neuropsychology & Neuropsychological Assessment',de:'Neuropsychologie und neuropsychologische Diagnostik',ru:'Нейропсихология и нейропсихологическая оценка'},
 {key:'tomatis',ar:'توماتيس والعلاج السمعي — مراجعة تعليمية',en:'Tomatis and Auditory Training — Educational Review',de:'Tomatis und auditives Training — Bildungsprogramm',ru:'Метод Томатис и слуховой тренинг — учебный курс'},
 {key:'sensory',ar:'التكامل الحسي والعلاج الحسي الحركي',en:'Sensory Integration & Sensorimotor Therapy',de:'Sensorische Integration und sensomotorische Therapie',ru:'Сенсорная интеграция и сенсомоторная терапия'},
 {key:'denver',ar:'أسلوب دنفر للأطفال والتدخل النمائي المبكر',en:'Denver Model & Early Developmental Intervention',de:'Denver-Modell und frühe Entwicklungsförderung',ru:'Денверская модель и раннее вмешательство'},
 {key:'intellectual',ar:'التعامل مع الإعاقة الذهنية ودعم المهارات التكيفية',en:'Intellectual Disability & Adaptive Skills Support',de:'Geistige Behinderung und adaptive Fähigkeiten',ru:'Интеллектуальная недостаточность и адаптивные навыки'},
 {key:'violence',ar:'العنف والسلوك العدواني عند الأطفال — التقييم والتدخل',en:'Child Aggression & Violence — Assessment and Intervention',de:'Aggression und Gewalt bei Kindern — Abklärung und Intervention',ru:'Агрессия и насилие у детей — оценка и вмешательство'},
 {key:'autism-violence',ar:'العنف والسلوكيات الصعبة مع التوحد',en:'Aggression and Challenging Behavior in Autism',de:'Aggression und herausforderndes Verhalten bei Autismus',ru:'Агрессия и сложное поведение при аутизме'},
 {key:'child-psych',ar:'علم نفس الطفل من النمو إلى التدخل',en:'Child Psychology: Development to Intervention',de:'Kinderpsychologie: Entwicklung und Intervention',ru:'Детская психология: развитие и вмешательство'},
 {key:'cbt',ar:'العلاج المعرفي السلوكي CBT — أساسيات وتطبيقات',en:'CBT — Foundations and Applications',de:'KVT — Grundlagen und Anwendungen',ru:'КПТ — основы и применение'},
 {key:'assessment',ar:'التقييم النفسي والاختبارات السريرية',en:'Psychological Assessment and Clinical Testing',de:'Psychologische Diagnostik und klinische Tests',ru:'Психологическая оценка и клиническое тестирование'},
 {key:'rehab',ar:'التأهيل الشامل للأطفال والبالغين',en:'Comprehensive Rehabilitation for Children and Adults',de:'Umfassende Rehabilitation für Kinder und Erwachsene',ru:'Комплексная реабилитация детей и взрослых'}
];
export function featuredCourses(lang:string,count=12):Course[]{const doc=virtualDoctorsForSpecialty('clinical-psychology',lang,5)[0];const spc=sp(lang,'clinical-psychology');return FEATURED_COURSE_TOPICS.slice(0,count).map((x,i)=>({id:'featured-course-'+lang+'-'+x.key,specialty_id:spc.id,doctor_id:doc.id,title:(x as any)[lang]||(x as any).en,description:lang==='ar'?'دورة تعليمية موسعة تتضمن وحدات نظرية وتطبيقات عملية ودروساً مكتوبة وفيديوهات وتمارين وملفات مراجعة. المحتوى مولد بمساعدة الذكاء الاصطناعي ويحتاج مراجعة أكاديمية قبل الاعتماد.':'Comprehensive educational course with theory, practical applications, written lessons, video lessons, exercises and references. AI-assisted draft requiring academic review before publication.',image_url:'https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=900&q=80',price:i<2?49:29,duration_weeks:i===0?24:8,lessons_count:i===0?120:40,enrolled_count:300+i*77,rating:4.8,is_published:true,created_at:new Date(2026,3,1+i).toISOString(),doctor,specialty:spc} as Course))};
